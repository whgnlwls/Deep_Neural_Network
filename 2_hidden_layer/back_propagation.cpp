...
void AI::setNet(string location) {
	...
	if (location == "hid1") {
	...
	else if (location == "hid2") {
		for (int i = 0; i < NUM_OF_HID2_NODE; i++) {
			total = 0;
			for (int j = 0; j < NUM_OF_HID1_NODE; j++) {
				total += out_H1[j] * weight_H1toH2[j][i];
			}
			net_H2[i] = total;
		}
	}
		...
		for (int i = 0; i < NUM_OF_OUT_NODE; i++) {
			total = 0;
			for (int j = 0; j < NUM_OF_HID2_NODE; j++) {
				total += out_H2[j] * weight_H2toO[j][i];
			}
			net_O[i] = total;
		}
	}
...
void AI::setError(string location, int dataNum) {
	if (location == "hid1") {
		...
		for (int i = 0; i < NUM_OF_HID1_NODE; i++) {
			total = 0;
			for (int j = 0; j < NUM_OF_HID2_NODE; j++) {
				total += weight_H1toH2_T[i][j] * del_H2[j];
			}
			err_H1[i] = total;
		}
	}
		...
		for (int i = 0; i < NUM_OF_HID2_NODE; i++) {
			total = 0;
			for (int j = 0; j < NUM_OF_OUT_NODE; j++) {
				total += weight_H2toO_T[i][j] * del_O[j];
			}
			err_H2[i] = total;
		}
	}
...
void AI::setDelta(string location) {
	if (location == "hid1") {
		...
		for (int i = 0; i < NUM_OF_HID2_NODE; i++) {
			del_H2[i] = getSigmoidPrime(net_H2[i]) * err_H2[i];
		}
	}
	else if (location == "out") {
		for (int i = 0; i < NUM_OF_OUT_NODE; i++) {
...
void AI::setWeight(string location) {
	...
	else if (location == "H1toH2") {
		for (int i = 0; i < NUM_OF_HID1_NODE; i++) {
			for (int j = 0; j < NUM_OF_HID2_NODE; j++) {
				deltaWeight = ALPHA * out_H1[i] * del_H2[j];
				weight_H1toH2[i][j] = weight_H1toH2[i][j] + deltaWeight;
			}
		}
	}
	else if (location == "H2toO") {
		for (int i = 0; i < NUM_OF_HID2_NODE; i++) {
			for (int j = 0; j < NUM_OF_OUT_NODE; j++) {
				deltaWeight = ALPHA * out_H2[i] * del_O[j];
				weight_H2toO[i][j] = weight_H2toO[i][j] + deltaWeight;
			}
		}
	}
}
...
void AI::setTranspose(string location) {
	if (location == "H1toH2") {
		for (int i = 0; i < NUM_OF_HID1_NODE; i++) {
			for (int j = 0; j < NUM_OF_HID2_NODE; j++) {
				weight_H1toH2_T[j][i] = weight_H1toH2[i][j];
			}
		}
	}
	else if (location == "H2toO") {
		for (int i = 0; i < NUM_OF_HID2_NODE; i++) {
			for (int j = 0; j < NUM_OF_OUT_NODE; j++) {
				weight_H2toO_T[j][i] = weight_H2toO[i][j];
			}
		}
	}
}
